...
func TestFPP(t *testing.T) {
	n := 10000
	fptarget := 0.001
	f := NewWithEstimates(uint(n), fptarget)
	...
	for i := uint32(0); i < uint32(n); i++ {
		np := make([]byte, 4)
		binary.BigEndian.PutUint32(np, i+uint32(n))
		if f.Test(np) {
			count += 1
		}
	}
	fprate := float64(count) / float64(n)
	if fprate > fptarget+(fptarget*0.01) { // 2% tolerance
		t.Errorf("False positive rate is %v but should be less than %v (+2 percent tolerance)", fprate, fptarget)
	}
}
